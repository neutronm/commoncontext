...
You are building two screens for a short demo that will be screen-recorded and submitted to Y Combinator. Read `00-demo-scope-and-beats.md`, `03-ui-spec.md`, and `04-repo-conventions.md` before writing anything. Doc 03's token names and semantic meanings, component anatomy, and copy are fixed; exact semantic color values and reduced-opacity styling must be adjusted as necessary to meet its authoritative 4.5:1 contrast requirement.
...
**Phase boundary:** this phase is mock-first UI work only. Do not open a database client or call domain functions from either page in this phase. The separate phase 2 dispatch owns DB wiring: it replaces the page-level mock imports with calls to the real domain layer while leaving the prop-driven components unchanged.
...
### Phase 2 DB-wiring notes (not part of this dispatch)

In phase 2, page-level server components open a postgres.js client from `getCloudflareContext().env.HYPERDRIVE.connectionString` and pass it into the domain functions (doc 04). Use one client per request, never a module-level singleton. If you see "getCloudflareContext has been called without having called initOpenNextCloudflareForDev", that's a `next.config.ts` problem — Fred's file, not yours. Flag it, don't fix it.

Phase 2 also adds `export const dynamic = 'force-dynamic';` to both pages because they then read a per-request Cloudflare binding and cannot be prerendered. Without it the build either fails or bakes a stale page.

In phase 2, participant names come from `getWorkspaceParticipants` and are passed down as a prop. In phase 1, the same fixed participant list comes from mock data. In both phases, the stance strip takes its names from the complete participant list, never from an object's `audienceNames`: a private object has an audience of one, so deriving from audience drops the `no access` cell on the cards that exist to show it.
...
Both pages resolve the viewer from `?as=fred|sara` (doc 04). There is no login. Every link must preserve the current value in phase 1; the real response POST must preserve it when phase 2 wires that action. A response recorded as the wrong founder is invisible on screen and ruins the take.
...
All strings come from doc 03 verbatim, including “Nothing becomes shared context until you respond.” and the line about her response being recorded alongside Fred's statement. Those sentences are the demo's argument stated in the interface. Do not rewrite them for tone. Buttons keep their name through the flow: `Record response` produces `Response recorded`.
...
Paste phase 1 screenshots for each:

1. `/workspace` viewing as Fred — exactly one dashed private card visible, `PRIVATE TO YOU` chip present
2. `/workspace` viewing as Sara — exactly one private card (her own P2), zero other-founder private cards, and no gap or placeholder where Fred's P1 would be
...
5. The mock pending review state, and the mock post-response state — the latter showing a populated stance strip in the timeline's layout
6. Screenshot 1 exported at 1080p and viewed at 50% size: every metadata line still readable
7. The mock review state with `?as=sara` — select a dispute and confirm the resulting stance strip reads `SARA · disputed` and not `FRED · disputed`
...
In phase 2, repeat checks 5 and 7 against a real pending object and the real response POST.

## Report back with

- All seven screenshots
- The token names and final contrast-verified values you used in CSS, plus the final superseded-text opacity, so doc 03 can be aligned
